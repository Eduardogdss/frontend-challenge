    ...
    return(
        <>
            <Layout>
                ...
                {(artista.name === "Não Encontrado" ) && <Redirect push to="/Busca"/>}
                {loading && <h1>carregando...</h1>}
                {artista!==[] && <h1>{artista.name}</h1>}
                {console.log(artista)}         
                ...
                {artista.images && artista.images.length>0 ?
                    <img height='100px' width = '100px' src={artista.images ? artista.images[0].url : facelessImage} alt ='Imagem do Artista'/> :
                ...
                {artista!==[] && <h2>Popularidade:{artista.popularity}</h2>}
                {albums!==[] && albums.map((album)=>
                <div>
                    <h1>{album.name}</h1>
                    <p>Data de Lançamento:{album.release_date}</p>
                    <img src={album.images ? album.images[0].url : ''} alt={album.name} height='100px' width='100px'/>
                </div>
                )}
            </Layout>
        </>
